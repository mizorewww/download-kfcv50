...
CHATGPT_PACKAGES = [
    ('ChatGPT-mac-arm64.dmg', 'macOS · Apple Silicon', 'macOS', '完整安装包 · Apple Silicon / arm64', '完整包', False),
    ('ChatGPT-mac-installer-web.dmg', 'macOS · 在线安装器', 'macOS', '自动选择适配的 Mac 版本 · 需联网', '官网安装器', False),
    ('ChatGPT-windows-x64.msix', 'Windows · x64', 'Windows', '完整离线包 · Intel / AMD 64 位', '完整包', False),
    ('ChatGPT-windows-installer-web.exe', 'Windows · 在线安装器', 'Windows', 'Microsoft Store 安装器 · 需联网', '官网安装器', False),
...
def item(path, product, platform, title, subtitle, badge='', warning=False):
...
for filename, title, platform, subtitle, badge, warning in CHATGPT_PACKAGES:
    ...
    packages.append(item(path, 'chatgpt', platform, title, subtitle, badge, warning))
...
for path in sorted(FLCLASH.glob('FlClash-*')):
    ...
    packages.append(item(path, 'flclash', platform_key, title, subtitle, badge))
